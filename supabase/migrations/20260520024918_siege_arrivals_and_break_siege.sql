-- Migration: siege_arrivals_and_break_siege
-- Introduces full siege system:
--   • process_ticks: siege arrival scenarios (begin siege, reinforce, relief army,
--     fight through enemy siege, siege bleed 3%/tick, infra damage per cycle)
--   • break_siege_and_assault: player-triggered assault after ≥1 production cycle of siege

-- ============================================================
-- break_siege_and_assault RPC
-- ============================================================
CREATE OR REPLACE FUNCTION public.break_siege_and_assault(
  p_game_id    bigint,
  p_castle_slug text
)
RETURNS jsonb
...
BEGIN
  ...
  -- Attacker: Hand × (1 + cmd_level × 0.1)
  -- Defender: (Hand+1) × (1 + cmd_level × 0.1)  (+1 for castle bonus)
...
-- ============================================================
-- process_ticks — full rewrite with siege support
-- See inline comments for siege-specific sections
-- ============================================================
    ...
    IF v_tick % v_cycle_len = 0 THEN
      ...
      FOR v_player IN SELECT gp.* FROM game_players gp WHERE gp.game_id=p_game_id AND gp.eliminated_at_tick IS NULL LOOP
        ...
        IF v_gm_lv > 0 THEN
          ...
          BEGIN
            ...
            IF eligible IS NOT NULL AND array_length(eligible,1)>0 THEN
              ...
              IF v_rand_prog >= v_rand_cost THEN UPDATE council_seats SET level=level+1, research_progress=0 WHERE game_id=p_game_id AND player_id=v_player.id AND seat=v_rand_seat; END IF;
            END IF;
          END;
        END IF;
        ...
        VALUES (p_game_id,v_player.id,v_tick,'production_cycle','Castellan Report',
          jsonb_build_object('gift_type','production_cycle','cycle_number',v_tick/v_cycle_len,'castles_owned',v_castles_owned,'gold_level',v_total_gold_lv,'prestige_level',v_total_prestige_lv,'coin_level',v_coin_lv,'gold_earned',v_gold_earned,'prestige_earned',v_prestige_earned));
      END LOOP;
    END IF;
